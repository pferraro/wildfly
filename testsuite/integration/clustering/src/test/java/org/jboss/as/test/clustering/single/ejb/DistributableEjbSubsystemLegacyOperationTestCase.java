    ...
    public static class ServerSetupTask extends CLIServerSetupTask {
        public ServerSetupTask() {
            this.builder.node("single")
                    ...
                    .setup("/subsystem=ejb3/cache=simple:add()")
                    .setup("/subsystem=ejb3/passivation-store=infinispan:add(cache-container=ejb, max-size=10000")
                    .setup("/subsystem=ejb3/cache=distributable:add(passivation-store=infinispan,aliases=[passivating,clustered])")
                    // remove legacy cache support
                    .teardown("/subsystem=ejb3/cache=simple:remove(){allow-resource-service-restart=true}")
                    .teardown("/subsystem=ejb3/cache=distributable:remove(){allow-resource-service-restart=true}")
                    ...
                    .teardown("/subsystem=distributable-ejb/infinispan-bean-management=default:add(cache-container=ejb,cache=passivation,max-size=10000)")
